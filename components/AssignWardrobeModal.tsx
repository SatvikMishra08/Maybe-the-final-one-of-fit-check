...
// FIX: Import ProductGarment type
import { WardrobeItem, ProductGarment } from '../types';
...
interface AssignWardrobeModalProps {
  ...
  // FIX: Update onSave to expect ProductGarment[]
  onSave: (outfit: ProductGarment[]) => void;
}
...
const AssignWardrobeModal: React.FC<AssignWardrobeModalProps> = ({ isOpen, onClose, wardrobe, onSave }) => {
  ...
  const handleSave = () => {
    // FIX: Map selected WardrobeItems to ProductGarment, assuming the first variant.
    const selectedOutfit: ProductGarment[] = wardrobe
      .filter(item => selectedIds.has(item.id))
      .map(item => ({
        garmentId: item.id,
        variantId: item.variants[0].id, // Default to the first variant
      }));
    onSave(selectedOutfit);
    onClose();
  };
